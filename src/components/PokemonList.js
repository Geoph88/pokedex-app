...
function PokemonList({
  ...
  const [pageCount, setPageCount] = useState(57)
  ...
  useEffect(() => {
    ...
    const response = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=1133`).then(res => res.json())

    const newPokemonData = response.results
    const endOffset = offset + pokemonPerPage
  ...
  const handleChange = (event, value) => {
    let startOffset = pokemonPages[value] - pokemonPerPage
        ...
        <Card sx={{ 
          ...
          <div className='pokemon-container' key={index}>
            ...
            style={{
              ...
              <Link to={`/PokemonDetails/${pokemon.name}`} style={{color: 'black', marginTop: '10px'}}>
              <CardMedia
              component='img'
              height='140'
